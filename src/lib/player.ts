...
import type {StreamVariant, VideoSession} from "./media-types.js";

export type PlayerVo = "auto" | "kitty" | "sixel" | "tct";

export type PlayerSupport = {
  mpvInstalled: boolean;
  ffplayInstalled: boolean;
  preferredVo: Exclude<PlayerVo, "auto">;
  detectedTerminal: string;
  notes: string[];
};

export function detectPlayerSupport(preferredVo: PlayerVo = "auto"): PlayerSupport {
  const mpvInstalled = hasCommand("mpv");
  const ffplayInstalled = hasCommand("ffplay");
  const detectedTerminal = detectTerminalLabel();

  const notes: string[] = [];
  let resolvedVo: Exclude<PlayerVo, "auto"> = "tct";
...
function detectTerminalLabel(): string {
  const parts = [
...
function looksLikeSixelTerminal(): boolean {
  const term = process.env.TERM?.toLowerCase() ?? "";
  const termProgram = process.env.TERM_PROGRAM?.toLowerCase() ?? "";
